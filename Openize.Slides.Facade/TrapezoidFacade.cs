...
namespace Openize.Slides.Facade
{
    public class TrapezoidFacade
    {
        ...
        public P.Shape Trapezoid { get => _Trapezoid; set => _Trapezoid = value; }
        ...
        public TrapezoidFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public TrapezoidFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public TrapezoidFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.ShapeProperties CreateShapeProperties(long x, long y, long width, long height, string rgbColorHex = "Transparent")
        {
            ...
            D.PresetGeometry presetGeometry1 = new D.PresetGeometry() { Preset = D.ShapeTypeValues.Trapezoid };
        ...
        private P.NonVisualShapeProperties CreateNonVisualShapeProperties()
        {
            ...
            P.NonVisualDrawingProperties nonVisualDrawingProperties1 = new P.NonVisualDrawingProperties() { Id = (UInt32Value)9U, Name = "Trapezoid 8" };
        ...
        public void UpdateShape()
        {
            ...
            Trapezoid.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            Trapezoid.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            Trapezoid.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            Trapezoid.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                Trapezoid.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (Trapezoid.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (Trapezoid.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            Trapezoid.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        Trapezoid.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (Trapezoid.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        Trapezoid.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = Trapezoid.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        Trapezoid.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = Trapezoid.TextBody.Descendants<Run>().FirstOrDefault();
            Trapezoid.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                Trapezoid.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            Trapezoid.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = Trapezoid.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == Trapezoid).ToList();
        ...
        private static bool IsTrapezoid(P.Shape shape)
        {
            var shapeProperties = shape.ShapeProperties;
            if (shapeProperties != null)
            {
                var presetGeometry = shapeProperties.GetFirstChild<D.PresetGeometry>();
                if (presetGeometry != null && presetGeometry.Preset == D.ShapeTypeValues.Trapezoid)
                {
                    return true;
                }
            }
        ...
        public static List<TrapezoidFacade> PopulateTrapezoids(SlidePart slidePart)
        {
            IEnumerable<P.Shape> shapes = slidePart.Slide.CommonSlideData.ShapeTree.Elements<P.Shape>();
            var rectangleShapes = new List<TrapezoidFacade>();
            var shapeIndex = 0;
            foreach (var shape in shapes)
            {
                if (IsTrapezoid(shape))
                {
                    var rectangleShapeFacade = new TrapezoidFacade
                    {
                        Trapezoid = shape, // Store the P.Shape in the private field
        ...
        private static TextAlignment GetAlignmentFromTrapezoid(P.Shape rectangleShape)
        {

...
{
    public class RectangleShapeFacade
    {
        ...
        public P.Shape RectangleShape { get => _RectangleShape; set => _RectangleShape = value; }
        ...
        public RectangleShapeFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public RectangleShapeFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public RectangleShapeFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.NonVisualShapeProperties CreateNonVisualShapeProperties()
        {
            ...
            P.NonVisualDrawingProperties nonVisualDrawingProperties1 = new P.NonVisualDrawingProperties() { Id = (UInt32Value)9U, Name = "Rectangle 8" };
        ...
        public void UpdateShape()
        {
            ...
            RectangleShape.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            RectangleShape.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            RectangleShape.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            RectangleShape.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                RectangleShape.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (RectangleShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (RectangleShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            RectangleShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        RectangleShape.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (RectangleShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        RectangleShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = RectangleShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        RectangleShape.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = RectangleShape.TextBody.Descendants<Run>().FirstOrDefault();
            RectangleShape.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                RectangleShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            RectangleShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = RectangleShape.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == RectangleShape).ToList();
        ...
        private static bool IsRectangleShape(P.Shape shape)
        {
            var shapeProperties = shape.ShapeProperties;
            if (shapeProperties != null)
            {
                var presetGeometry = shapeProperties.GetFirstChild<D.PresetGeometry>();
                if (presetGeometry != null && presetGeometry.Preset == D.ShapeTypeValues.Rectangle)
                {
                    return true;
                }
            }
        ...
        public static List<RectangleShapeFacade> PopulateRectangleShapes(SlidePart slidePart)
        {
            IEnumerable<P.Shape> shapes = slidePart.Slide.CommonSlideData.ShapeTree.Elements<P.Shape>();
            var rectangleShapes = new List<RectangleShapeFacade>();
            var shapeIndex = 0;
            foreach (var shape in shapes)
            {
                if (IsRectangleShape(shape))
                {
                    var rectangleShapeFacade = new RectangleShapeFacade
                    {
                        RectangleShape = shape, // Store the P.Shape in the private field
        ...
        private static TextAlignment GetAlignmentFromRectangleShape(P.Shape rectangleShape)
        {

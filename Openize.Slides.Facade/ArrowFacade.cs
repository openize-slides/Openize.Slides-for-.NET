...
namespace Openize.Slides.Facade
{
    public class ArrowFacade
    {
        ...
        public P.Shape ArrowShape { get => _ArrowShape; set => _ArrowShape = value; }
        ...
        public ArrowFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public ArrowFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public ArrowFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.ShapeProperties CreateShapeProperties(long x, long y, long width, long height, string rgbColorHex = "Transparent")
        {
            ...
            D.PresetGeometry presetGeometry1 = new D.PresetGeometry() { Preset = D.ShapeTypeValues.Line };
        ...
        public void UpdateShape()
        {
            ...
            ArrowShape.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            ArrowShape.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            ArrowShape.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            ArrowShape.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                ArrowShape.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (ArrowShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (ArrowShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            ArrowShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        ArrowShape.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (ArrowShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        ArrowShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = ArrowShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        ArrowShape.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = ArrowShape.TextBody.Descendants<Run>().FirstOrDefault();
            ArrowShape.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                ArrowShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            ArrowShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = ArrowShape.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == ArrowShape).ToList();
        ...
        private static bool IsArrowShape(P.Shape shape)
        {
        ...
        public static List<ArrowFacade> PopulateArrowShapes(SlidePart slidePart)
        {
            IEnumerable<P.Shape> shapes = slidePart.Slide.CommonSlideData.ShapeTree.Elements<P.Shape>();
            var rectangleShapes = new List<ArrowFacade>();
            var shapeIndex = 0;
            foreach (var shape in shapes)
            {
                if (IsArrowShape(shape))
                {
                    var rectangleShapeFacade = new ArrowFacade
                    {
                        ArrowShape = shape, // Store the P.Shape in the private field
        ...
        private static TextAlignment GetAlignmentFromArrowShape(P.Shape rectangleShape)
        {

...
namespace Openize.Slides.Facade
{
    public class HexagonFacade
    {
        ...
        public P.Shape Hexagon { get => _Hexagon; set => _Hexagon = value; }
        ...
        public HexagonFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public HexagonFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public HexagonFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.ShapeProperties CreateShapeProperties(long x, long y, long width, long height, string rgbColorHex = "Transparent")
        {
            ...
            D.PresetGeometry presetGeometry1 = new D.PresetGeometry() { Preset = D.ShapeTypeValues.Hexagon };
        ...
        private P.NonVisualShapeProperties CreateNonVisualShapeProperties()
        {
            ...
            P.NonVisualDrawingProperties nonVisualDrawingProperties1 = new P.NonVisualDrawingProperties() { Id = (UInt32Value)9U, Name = "Hexagon 8" };
        ...
        public void UpdateShape()
        {
            ...
            Hexagon.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            Hexagon.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            Hexagon.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            Hexagon.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                Hexagon.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (Hexagon.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (Hexagon.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            Hexagon.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        Hexagon.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (Hexagon.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        Hexagon.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = Hexagon.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        Hexagon.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = Hexagon.TextBody.Descendants<Run>().FirstOrDefault();
            Hexagon.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                Hexagon.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            Hexagon.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = Hexagon.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == Hexagon).ToList();
        ...
        private static bool IsHexagon(P.Shape shape)
        {
            var shapeProperties = shape.ShapeProperties;
            if (shapeProperties != null)
            {
                var presetGeometry = shapeProperties.GetFirstChild<D.PresetGeometry>();
                if (presetGeometry != null && presetGeometry.Preset == D.ShapeTypeValues.Hexagon)
                {
                    return true;
                }
            }
        ...
        public static List<HexagonFacade> PopulateHexagons(SlidePart slidePart)
        {
            IEnumerable<P.Shape> shapes = slidePart.Slide.CommonSlideData.ShapeTree.Elements<P.Shape>();
            var rectangleShapes = new List<HexagonFacade>();
            var shapeIndex = 0;
            foreach (var shape in shapes)
            {
                if (IsHexagon(shape))
                {
                    var rectangleShapeFacade = new HexagonFacade
                    {
                        Hexagon = shape, // Store the P.Shape in the private field
        ...
        private static TextAlignment GetAlignmentFromHexagon(P.Shape rectangleShape)
        {

...
{
    public class DoubleBraceFacade
    {
        ...
        private P.Shape _DoubleBraceShape;
        ...
        public P.Shape DoubleBraceShape { get => _DoubleBraceShape; set => _DoubleBraceShape = value; }
        ...
        public DoubleBraceFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public DoubleBraceFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public DoubleBraceFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.ShapeProperties CreateShapeProperties(long x, long y, long width, long height, string rgbColorHex = "Transparent")
        {
            ...
            D.PresetGeometry presetGeometry1 = new D.PresetGeometry() { Preset = D.ShapeTypeValues.BracketPair };
        ...
        public void UpdateShape()
        {
            ...
            DoubleBraceShape.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            DoubleBraceShape.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            DoubleBraceShape.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            DoubleBraceShape.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                DoubleBraceShape.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (DoubleBraceShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (DoubleBraceShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            DoubleBraceShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        DoubleBraceShape.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (DoubleBraceShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        DoubleBraceShape.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = DoubleBraceShape.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        DoubleBraceShape.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = DoubleBraceShape.TextBody.Descendants<Run>().FirstOrDefault();
            DoubleBraceShape.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                DoubleBraceShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            DoubleBraceShape.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = DoubleBraceShape.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == DoubleBraceShape).ToList();
        ...
        private static bool IsDoubleBraceShape(P.Shape shape)
        {
            var shapeProperties = shape.ShapeProperties;
            if (shapeProperties != null)
            {
                var presetGeometry = shapeProperties.GetFirstChild<D.PresetGeometry>();
                if (presetGeometry != null && presetGeometry.Preset == D.ShapeTypeValues.BracketPair)
                {
                    return true;
                }
            }
        ...
        public static List<DoubleBraceFacade> PopulateDoubleBraceShapes(SlidePart slidePart)
        {
            IEnumerable<P.Shape> shapes = slidePart.Slide.CommonSlideData.ShapeTree.Elements<P.Shape>();
            var rectangleShapes = new List<DoubleBraceFacade>();
            var shapeIndex = 0;
            foreach (var shape in shapes)
            {
                if (IsDoubleBraceShape(shape))
                {
                    var rectangleShapeFacade = new DoubleBraceFacade
                    {
                        DoubleBraceShape = shape, // Store the P.Shape in the private field
        ...
        private static TextAlignment GetAlignmentFromDoubleBraceShape(P.Shape rectangleShape)
        {

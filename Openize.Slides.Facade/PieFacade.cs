...
namespace Openize.Slides.Facade
{
    public class PieFacade
    {
        ...
        public P.Shape Pie { get => _Pie; set => _Pie = value; }
        ...
        public PieFacade WithBackgroundColor(String backgroundColor)
        {
        ...
        public PieFacade WithPosition(long x, long y)
        {
            X = x;
            Y = y;
            return this;
        ...
        public PieFacade WithSize(long width, long height)
        {
            Width = width;
            Height = height;
            return this;
        ...
        private P.ShapeProperties CreateShapeProperties(long x, long y, long width, long height, string rgbColorHex = "Transparent")
        {
            ...
            D.PresetGeometry presetGeometry1 = new D.PresetGeometry() { Preset = D.ShapeTypeValues.Pie };
        ...
        private P.NonVisualShapeProperties CreateNonVisualShapeProperties()
        {
            ...
            P.NonVisualDrawingProperties nonVisualDrawingProperties1 = new P.NonVisualDrawingProperties() { Id = (UInt32Value)9U, Name = "Pie 8" };
        ...
        public void UpdateShape()
        {
            ...
            Pie.NonVisualShapeProperties.NonVisualDrawingProperties.Id = (UInt32Value)5U;
            Pie.NonVisualShapeProperties.NonVisualDrawingProperties.Name = "Text Box 1";
            Pie.NonVisualShapeProperties.NonVisualShapeDrawingProperties = new P.NonVisualShapeDrawingProperties(new ShapeLocks() { NoGrouping = true });
            Pie.NonVisualShapeProperties.ApplicationNonVisualDrawingProperties = new ApplicationNonVisualDrawingProperties(new PlaceholderShape());
            if (Width != 0)
            {
                Pie.ShapeProperties.Transform2D = new D.Transform2D()
                {
            ...
            if (!String.IsNullOrEmpty(_BackgroundColor))
            {
                ...
                {
                    if (Pie.ShapeProperties.Descendants<NoFill>().FirstOrDefault() == null)
                    {
                        if (Pie.ShapeProperties.Descendants<SolidFill>().FirstOrDefault() != null)
                            Pie.ShapeProperties.Descendants<SolidFill>().FirstOrDefault().Remove();
                    }
                    else
                    {
                        Pie.ShapeProperties.Append(new NoFill());
                    }
                ...
                else
                {
                    if (Pie.ShapeProperties.Descendants<NoFill>().FirstOrDefault() != null)
                    {
                        Pie.ShapeProperties.Descendants<NoFill>().FirstOrDefault().Remove();
                    }
                    var fill = Pie.ShapeProperties.Descendants<SolidFill>().FirstOrDefault();
                    ...
                    {
                        Pie.ShapeProperties.Append(new SolidFill(new RgbColorModelHex() { Val = _BackgroundColor }));
                    }

                }
            }

            var existingParagraphText = Pie.TextBody.Descendants<Run>().FirstOrDefault();
            Pie.TextBody.Elements<Paragraph>().FirstOrDefault().RemoveAllChildren();
            if (alignmentType != TextAlignmentTypeValues.Justified)
                Pie.TextBody.Elements<Paragraph>().FirstOrDefault().Append(new ParagraphProperties() { Alignment = alignmentType });
            Pie.TextBody.Elements<Paragraph>().FirstOrDefault().Append(existingParagraphText);

            var runProperties = Pie.TextBody.Descendants<RunProperties>().FirstOrDefault();
        ...
        public void RemoveShape(SlidePart slidePart)
        {
            ...
            if (commonSlideData != null && commonSlideData.ShapeTree != null)
            {
                ...
                var shapesToRemove = commonSlideData.ShapeTree.Elements<P.Shape>().Where(shape => shape == Pie).ToList();
        ...
        private static bool IsPie(P.Shape shape)
        {
            var shapeProperties = shape.ShapeProperties;
            if (shapeProperties != null)
            {
                var presetGeometry = shapeProperties.GetFirstChild<D.PresetGeometry>();
                if (presetGeometry != null && presetGeometry.Preset == D.ShapeTypeValues.Pie)
                {
                    return true;
                }
            }
